...
export default function LiderDashboard() {
  ...
  const cards = [
    { label: t("dashboard.lider.myGroups"), value: groups.length, icon: Layers },
    { label: t("dashboard.lider.currentWeek"), value: activeWeek ?? t("dashboard.lider.notStarted"), icon: Calendar },
  ];

  return (
    <DashboardLayout title={t("dashboard.lider.title")}>
      ...
      <div className="mt-6 glass-card overflow-hidden">
        <div className="p-4 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="section-label">{t("dashboard.lider.groupsUnderLeadership")}</span>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-8 flex flex-col items-center justify-center text-center space-y-3">
            ...
            <p className="text-muted-foreground">{t("dashboard.lider.noGroupsLinked")}</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-muted-foreground">
                  <th className="text-left px-4 py-3 font-medium">{t("dashboard.lider.name")}</th>
                  <th className="text-left px-4 py-3 font-medium">{t("dashboard.lider.members")}</th>
                  <th className="text-left px-4 py-3 font-medium">{t("dashboard.lider.representative")}</th>
                </tr>
              </thead>
              <tbody>
                {groups.map((group) => (
