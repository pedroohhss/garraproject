...
export default function AdminWeeksPage() {
  ...
  const handleSaveEdit = async () => {
    ...
    if (error) toast({ title: "Erro", description: error.message, variant: "destructive" });
    else { toast({ title: "Semana atualizada" }); await loadWeeks(); }
    setSaving(false);
    setEditTarget(null);
  };
  ...
  const handleSaveCriterion = async () => {
    ...
    if (editCriterionId) {
      ...
      if (error) toast({ title: "Erro", description: error.message, variant: "destructive" });
      else toast({ title: "Critério atualizado" });
    } else {
      const { error } = await supabase.from("checklist_criteria").insert(payload);
      if (error) toast({ title: "Erro", description: error.message, variant: "destructive" });
      else toast({ title: "Critério criado" });
    }
    setCriteriaForm({ description: "", points: "10" });
    setEditCriterionId(null);
  ...
  const populateDefaults = async () => {
    ...
    toast({ title: "Critérios padrão adicionados" });
    await loadWeeks();
  };
    ...
    <DashboardLayout title="Gestão de Semanas">
            ...
            return (
              ...
              >
                ...
                <div className="text-xs text-muted-foreground">
                  ...
                  {wCriteria.length} critério(s) · {wCriteria.reduce((s, c) => s + (c.points ?? 0), 0)} pts total
                </div>
                <div className="flex gap-2 pt-1 flex-wrap">
                  ...
                  <Button size="sm" variant="ghost" onClick={() => openCriteria(week)} className="gap-1 text-xs">
                    <ClipboardCheck className="h-3 w-3" /> Critérios
                  </Button>
                  {status === "futura" && (
      ...
      <Dialog open={!!activateTarget} onOpenChange={(o) => !o && setActivateTarget(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Isso vai desativar qualquer outra semana ativa e tornar a Semana {activateTarget?.number} a semana atual. Critérios padrão serão adicionados se não existirem.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={!!criteriaWeek} onOpenChange={(o) => !o && setCriteriaWeek(null)}>
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Critérios — Semana {criteriaWeek?.number}</DialogTitle>
            <DialogDescription>
              {weekHasEntries
                ? "Grupos já submeteram o checklist. Critérios são somente leitura."
                : "Defina os critérios de avaliação e seus pesos em pontos."}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-2">
            {weekCriteria.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">Nenhum critério ainda.</p>
            ) : (
              weekCriteria.map((c) => (
            ...
            <div className="space-y-3 border-t border-border pt-4">
              <div className="space-y-1.5">
                <label className="text-xs text-muted-foreground font-medium">
                  {editCriterionId ? "Editar critério" : "Novo critério"}
                </label>
                <Input
              ...
              <div className="flex gap-2">
                <div className="space-y-1.5 w-24">
                  <label className="text-xs text-muted-foreground font-medium">Pontos</label>
                  <Input
                    type="number"
                ...
                <div className="flex items-end gap-2 flex-1">
                  <Button size="sm" onClick={handleSaveCriterion} disabled={savingCriterion || !criteriaForm.description.trim()}>
                    ...
                    {editCriterionId ? "Salvar" : "Adicionar"}
                  </Button>
                  {editCriterionId && (
              ...
              {weekCriteria.length === 0 && (
                <Button variant="outline" size="sm" onClick={populateDefaults} className="w-full gap-1">
                  <Plus className="h-3 w-3" /> Preencher critérios sugeridos
                </Button>
              )}
            </div>
          )}
      ...
      <AlertDialog open={!!deleteCriterion} onOpenChange={(o) => !o && setDeleteCriterion(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>"{deleteCriterion?.description}" será removido.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteCriterion} className="bg-destructive text-destructive-foreground">Remover</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </DashboardLayout>
